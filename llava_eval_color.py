...
# --- 配置路径 ---
BASE_DIR = "/home/mcy/flux/exp_results/T2I_CompBench_color/baseline/samples"
ADAP_DIR = "/home/mcy/flux/exp_results/T2I_CompBench_color/adaptive/samples"
MODEL_ID = "llava-hf/llava-1.5-7b-hf"
OUTPUT_JSON = "color_eval_paper_summary.json"

print(">>> 正在加载 LLaVA-1.5-7B 模型 (Color Binding)...")
...
def get_score_from_llava(image_path, prompt_text):
    ...
    instruction = (
        ...
        f"Evaluate the COLOR BINDING based on this prompt: '{prompt_text}'.\n\n"
        "Rubric:\n"
        "3 - Perfect: All objects have the exact colors specified. No color leakage to other objects or background.\n"
        "2 - Minor Leakage: Objects have the correct colors, but there is slight color bleeding to the background, or the color is slightly inaccurate.\n"
        "1 - Wrong Binding: The colors are present in the image but applied to the wrong objects (e.g., prompt says 'red apple, blue cup', image shows 'blue apple, red cup').\n"
        "0 - Missing: Key objects or the requested colors are completely missing from the image.\n\n"
        "Step 1: Briefly describe the objects and their specific colors in the image.\n"
...
def main():
    ...
    print(f">>> 开始对 {len(base_files)} 个样本进行 Color 深度交叉打分...")
    ...
    print(f"\n【学术评测报告 - Color Binding (CoT-based)】")
